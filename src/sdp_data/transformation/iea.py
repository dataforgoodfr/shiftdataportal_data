...
class EiaScrapper:  # TODO - à refactorer
    ...
    def __init__(self) -> None:
        ...
        self.list_countries_fix = {"AUSTRALI":"Australia",
                                   ...
                                   "KOREA":"South Korea","KOREADPR":"North Korea","MOLDOVA":"Moldova","NETHLAND":"Netherlands","NZ":"New Zealand","RUSSIA":"Russian Federation","SAUDIARABI":"Saudi Arabia","SOUTHAFRIC":"South Africa","SRILANKA":"Sri Lanka","SSUDAN":"South Sudan","SYRIA":"Syria","TAIPEI":"Taiwan","TANZANIA":"Tanzania","UK":"United Kingdom","USA":"United States","VIETNAM":"Viet Nam","VENEZUELA":"Venezuela"}
        ...
        for country in result:
            for year in result[country]:
                for g in result[country][year]:
                    c = self.country_fix.get(country,country)
...
class EiaDataProcessor:
    ...
    def convert_energy_ktoe_to_mtoe(self, final_energy: float, final_energy_unit: str):
        if final_energy_unit == "Ktoe":
            return final_energy * 0.001
        else:
            return final_energy
        
    def convert_energy_gigawatt_to_terrawatt(self, final_energy: float, final_energy_unit: str):
    ...
    def select_and_sort_values(df_eia_energy: pd.DataFrame):
        ...
        list_sort_values = ["group_type", "group_name", "year", "sector", "energy_family, final_energy_unit", "original_dataset"]
    ...
    def prepare_data(self, df_eia_data: pd.DataFrame, df_country: pd.DataFrame):
        """
        ...
        df_eia_data["final_energy"] = df_eia_data.apply(lambda row: self.convert_energy_ktoe(row["final_energy"], row["final_energy_unit"]), axis=1)
        ...
        list_cols_group_by = ["group_type", "group_name", "year", "sector", "energy_family", "final_energy_unit", "original_dataset"]
...
class EiaConsumptionGasBySectorProcessor:
...
class EiaElectricityGenerationByEnergyProcessor(EiaDataProcessor):
    ...
    def compute_nuclear_share_in_electricity(self):
        ...
        df_electricity_total = df_electricity_by_energy_family.groupby(list_group_by).agg({'final_energy': 'sum'}).reset_index().reset_index()
        ...
        df_nuclear_electricity_share = df_electricity_nuclear.merge(df_electricity_total, on=['group_type', 'group_name', 'year', 'final_energy_unit', 'source'], how='inner')
        df_nuclear_electricity_share['nuclear_share_of_electricity_generation'] = df_nuclear_electricity_share['nuclear'] / df_nuclear_electricity_share['final_energy_total']
        df_nuclear_electricity_share = df_nuclear_electricity_share[["group_type", "group_name", "year", "nuclear", "nuclear_share_of_electricity_generation"
                                                                      "final_energy_unit", "source"]]
        df_nuclear_electricity_share = StatisticsDataframeFormatter().select_and_sort_values(df_nuclear_electricity_share, "nuclear_share_of_electricity_generation")
